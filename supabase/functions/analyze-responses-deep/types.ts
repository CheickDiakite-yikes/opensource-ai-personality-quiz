...
// REVISED: Prompt expects string[]
export type ValueSystemType = string[];

// Cognitive Style type to handle both string and object
// REVISED: Prompt expects object
export interface CognitiveStyleType { 
  primary: string;
  secondary: string;
  description: string;
}
...
export interface PersonalityAnalysis {
  ...
  cognitiveStyle: CognitiveStyleType; // Use interface, not union type
  valueSystem: ValueSystemType; // Use revised type (string[])
  motivators: string[];
  inhibitors: string[];
  weaknesses: string[];
  shadowAspects: string[]; // ADDED missing field
  growthAreas: string[];
  relationshipPatterns: RelationshipPatterns; // Use interface, not union type
