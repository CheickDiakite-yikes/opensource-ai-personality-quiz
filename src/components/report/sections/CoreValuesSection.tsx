  ...
  return (
    <Card className="glass-panel overflow-hidden">
      ...
      <CardContent className={`${isMobile ? 'px-3 py-2' : 'pt-3 md:pt-6'}`}>
        <div className={`grid ${isMobile ? 'grid-cols-1 gap-1.5' : 'sm:grid-cols-2 gap-2'} max-w-full`}>
          {valuesToDisplay.map((value, index) => (
            ...
            >
              ...
              <span className={`${isMobile ? 'text-xs' : 'text-sm md:text-base'} break-words text-wrap`}>{value}</span>
            </div>
          ))}
        </div>
      </CardContent>
