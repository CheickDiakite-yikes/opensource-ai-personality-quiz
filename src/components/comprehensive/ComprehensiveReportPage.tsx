...
import { AlertTriangle, RefreshCw, ArrowLeft, Share2 } from "lucide-react";
...
const ComprehensiveReportPage = () => {
  ...
  // Use the fallback hook for polling and recovery
  const { 
    pollForAnalysis, 
  ...
  const fetchAnalysis = async () => {
    ...
    try {
      // First try to get the analysis directly from the edge function
      console.log(`Fetching comprehensive analysis with ID: ${id}`);
      const edgeFunctionResult = await fetchWithEdgeFunction(id);
      
      if (edgeFunctionResult) {
        console.log("Successfully fetched analysis via edge function");
        setAnalysis(edgeFunctionResult);
        return;
      }
      
      console.log("Edge function fetch failed, trying direct database query");
      ...
      if (!data) {
        console.log("No analysis found, starting polling");
        // If no analysis found, start polling
        const polledAnalysis = await pollForAnalysis(id);
        
        if (polledAnalysis) {
          setAnalysis(polledAnalysis);
        } else {
          throw new Error("Analysis not found after polling");
        }
      } else {
      ...
      // If we haven't tried polling yet, try it as a last resort
      if (!hasAttemptedPolling && id) {
        console.log("Attempting polling as last resort");
        toast.loading("Attempting to retrieve your analysis...");
        ...
        try {
          ...
          console.error("Polling also failed:", pollError);
          toast.error("Could not retrieve analysis");
        }
      }
    } finally {
      setIsLoading(false);
  ...
  // Fetch analysis when component mounts or ID changes
  useEffect(() => {
    if (id) {
      fetchAnalysis();
    }
  ...
  // Update local state when polling finds an analysis
  useEffect(() => {
    if (foundAnalysis && !analysis) {
      console.log("Setting analysis from polling result");
    ...
    <div className="container py-8 px-4">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground">Your detailed personality profile</p>
        </div>
      ...
      {isLoading || isPolling ? (
        <Card className="p-8">
          <div className="text-center">
            ...
            <p className="mt-4 text-muted-foreground">
              {isPolling 
                ? `Retrieving your analysis... (Attempt ${pollingAttempts}/5)` 
                : "Loading your comprehensive analysis..."}
            </p>
          </div>
        </Card>
      ) : error ? (
        ...
        <Card className="p-8 text-center">
          ...
          <p className="text-muted-foreground mb-6">
            We couldn't find the comprehensive analysis you're looking for.
          </p>
      ...
      {conversionError && (
        <Card className="p-4 mt-8 bg-red-50 dark:bg-red-900/20">
          <h3 className="text-lg font-medium text-red-700 dark:text-red-300">Conversion Error</h3>
