...
// Improved analysis generator that considers actual responses
const generateAnalysisFromResponses = (responses: DeepInsightResponses): AnalysisData => {
  ...
  // Customize analysis based on response patterns
  let primaryTrait, secondaryTrait, analyticalScore, emotionalScore, adaptabilityScore;
  ...
  // Secondary trait based on second most common response
  switch(secondaryChoice) {
    case 'a':
      secondaryTrait = "Systematic Thinker";
      break;
    case 'b':
      secondaryTrait = "Relationship Focused";
      break;
    case 'c':
      secondaryTrait = "Pragmatic Adapter";
      break;
    case 'd':
      secondaryTrait = "Creative Explorer";
      break;
  }
  
  // Further customize based on specific response combinations
  const uniqueId = `analysis-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
  const now = new Date().toISOString();
  
  // Create personalized strengths and challenges based on response pattern
  const strengths = [];
  const challenges = [];
  const growthAreas = [];
